...
class VolumeWeightedAveragePrice(bt.Indicator):
    ...
    def __init__(self):
        ...
        cumvol = bt.If(cumvol==0, 1, cumvol)
...
class St(bt.Strategy):
    ...
    def __init__(self):
        ...
        vwap1, vwap2 = self.p.vwap(period=self.p.p3), \
                       self.p.vwap(period=self.p.p4)
        self.cross_ma = bt.ind.CrossOver(ma1, ma2)
        self.cross_vwap = bt.ind.CrossOver(vwap1, vwap2)
    ...
    params = dict(
        ma=bt.ind.ExponentialMovingAverage,
    ...
    def next(self):
        ...
        if not self.position:
            if self.cross_ma > 0.0:  # crossing up
                if self.cross_vwap > 0.0:
            ...
            # Sell Short
            elif self.cross_ma < 0.0:  # crossing up:
                if self.cross_vwap < 0.0:
                    close = self.data.close[0]
                    p1_sell = close * (1.0 + self.p.limit)
                    p2_sell = p1_sell + 0.01 * close
...
quantstats.reports.html(returns, output=r'D:\crypto_trading\Strategy_3\{}\Results.html'.format(date_mode),
...
returns.to_csv(r'D:\crypto_trading\Strategy_3\{}\Returns.csv'.format(date_mode))
positions.to_csv(r'D:\crypto_trading\Strategy_3\{}\Positions.csv'.format(date_mode))
transactions.to_csv(r'D:\crypto_trading\Strategy_3\{}\Transactions.csv'.format(date_mode))
gross_lev.to_csv(r'D:\crypto_trading\Strategy_3\{}\Gross_leverage.csv'.format(date_mode))
